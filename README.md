...
The techlab is available on GitHub Pages [here](https://puzzle-bbt.github.io/lerndoku/).
...
```bash
docker build -t tuxmatta/lerndoku:latest .
```

Run it locally:

```bash
docker run -i -p 8080:8080 tuxmatta/lerndoku
```
...
```bash
buildah build-using-dockerfile -t tuxmatta/lerndoku:latest .
```

Run it locally with the following command. Beware that `--rmi` automatically removes the built image when the container stops, so you either have to rebuild it or remove the parameter from the command.

```bash
podman run --rm --rmi --interactive --publish 8080:8080 localhost/tuxmatta/lerndoku:latest
```
